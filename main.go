...
func main() {
	...
	FullBinPath := filepath.Join(GlobalConf.Section1.GameBinDir, DSTExecName)
	...
	for {
		...
		for GlobalConf.AtomicGate.GameUpdatingGate.Load() || GlobalConf.AtomicGate.ModUpdatingGate.Load() {
		...
		if GlobalConf.Section2.EnableMaster.Load() {
			wg.Add(1)
			go BootMaster(LifeCtx, KillSwitch, FullBinPath)
		}
		if GlobalConf.Section2.EnableCaves.Load() {
			wg.Add(1)
			go BootCaves(LifeCtx, KillSwitch, FullBinPath)
		}
