...
func BootMaster(LifeCtx context.Context, KillSwitch context.CancelFunc, BinPath string) {
	defer wg.Done()
	defer KillSwitch()
	...
	defer cancelShard()
	MasterProc := exec.CommandContext(shardCtx, BinPath, append(GlobalConf.Section1.CommonBootArgs, "-shard", "Master")...)
	...
	defer vKillProcess(MasterProc, "Master", ProcExitSig, cancelShard, &GlobalConf.AtomicGate.MasterReadyGate)
	for {
	...
	defer func() {
		...
		GlobalConf.AtomicGate.MasterReadyGate.Store(false)
	}()
...
func BootCaves(LifeCtx context.Context, KillSwitch context.CancelFunc, BinPath string) {
	defer wg.Done()
	defer KillSwitch()
	...
	shardCtx, cancelShard := context.WithCancel(context.Background())
	defer cancelShard()
	CavesProc := exec.CommandContext(shardCtx, BinPath, append(GlobalConf.Section1.CommonBootArgs, "-shard", "Caves")...)
	...
	ProcExitSig := make(chan struct{})
	defer vKillProcess(CavesProc, "Caves", ProcExitSig, cancelShard, &GlobalConf.AtomicGate.CavesReadyGate)
	for {
		old := GlobalConf.ProcState.PID.Load()
	...
	defer func() {
		...
		GlobalConf.AtomicGate.CavesReadyGate.Store(false)
	}()
	...
	for {
		...
		for {
			...
			if bytes.HasPrefix(line, []byte("data: ")) {
				CurrPeerEpoch, err := strconv.ParseInt(string(bytes.TrimSpace(line[6:])), 10, 64)
				if err != nil {
					continue
				}
	...
	for {
		...
		if NodeType == Node_Master {
			...
			if Pos != -1 {
				...
				if bytes.Contains(PrefixChunk, S2B(Sig_PlayerChat)) {
...
func vKillProcess(cmd *exec.Cmd, ProcName string, DoomSensor <-chan struct{}, KillSwitch context.CancelFunc, ReadyFlag *atomic.Bool) {
	...
	if err != nil {
		LogOutLn(S2B("[warn] SIGINT failed, deploying SIGKILL immediately..."))
		KillSwitch()
		<-DoomSensor
	}

	TimeoutStopwatch := time.NewTimer(5 * time.Second)
	...
	case <-TimeoutStopwatch.C:
		LogOutLn(S2B("[warn] "), S2B(ProcName), S2B(" shutdown timeout (5s)! deploying SIGKILL..."))
		KillSwitch()
		<-DoomSensor
		LogOutLn(S2B("[core] "), S2B(ProcName), S2B(" has been purged from the OS."))
	}
