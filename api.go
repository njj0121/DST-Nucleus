...
func api_status(w http.ResponseWriter, r *http.Request) {
	if r.Method != "GET" {
		HttpErr(w, 400, S2B(`{"status":"error", "message":"method not allowed (GET required)"}`))
		return
	}
...
func api_events(w http.ResponseWriter, r *http.Request) {
	...
	if !CastOk {
		HttpErr(w, 500, S2B(`{"status":"error", "message":"flusher not supported"}`))
		return
	}
...
func api_command(w http.ResponseWriter, r *http.Request) {
	if !ApiCmdGate.LockState.CompareAndSwap(0, 1) {
		HttpErr(w, 423, S2B(`{"status":"error"}`))
		return
	}
	defer ApiCmdGate.LockState.Store(0)

	if r.Method != "POST" {
		HttpErr(w, 405, S2B(`{"status":"error", "message":"method not allowed (POST required)"}`))
		return
	}
	...
	for {
		if TotalRead == len(TmpBuffer) {
			HttpErr(w, 413, S2B(`{"status":"error", "message":"payload too large"}`))
			return
		}
		...
		if err != nil {
			...
			HttpErr(w, 400, S2B(`{"status":"error", "message":"bad request or payload too large"}`))
			return
		}
	}
	...
	if TotalRead == 0 {
		RxPool.Put(PooledPtr)
		HttpErr(w, 400, []byte(`{"status":"error", "message":"empty payload"}`))
		return
	}
	...
	case "all":
		...
		if MasterOk && CavesOk {
			...
			HttpErr(w, 503, []byte(`{"status":"warning", "message":"master ack, caves dropped (congestion)"}`))
			return
		} else if !MasterOk && CavesOk {
			HttpErr(w, 503, []byte(`{"status":"caves ack, master dropped (congestion)"}`))
			return
		} else {
			DeliverOk = false
		}
	...
	default:
		HttpErr(w, 400, []byte(`{"status":"error", "message":"invalid target"}`))
		return
	}

	if !DeliverOk {
		HttpErr(w, 503, []byte(`{"status":"error", "message":"pipeline congested, payload dropped"}`))
		return
	}
...
func api_start(w http.ResponseWriter, r *http.Request) {
	...
	default:
		HttpErr(w, 409, S2B(`{"status":"error", "message":"start blocked"}`))
		return
	}
	w.Header()["Content-Type"] = JsonHead
	w.Write(S2B(`{"status": "success"}`))
...
func api_file_read(w http.ResponseWriter, r *http.Request) {
	...
	switch Target {
	case "cluster":
		FilePath = filepath.Join(GlobalConf.Section1.StorageRoot, "DoNotStarveTogether", GlobalConf.Section1.ClusterName, "cluster.ini")
	case "master_server":
		FilePath = filepath.Join(GlobalConf.Section1.StorageRoot, "DoNotStarveTogether", GlobalConf.Section1.ClusterName, "Master", "server.ini")
	case "caves_server":
		FilePath = filepath.Join(GlobalConf.Section1.StorageRoot, "DoNotStarveTogether", GlobalConf.Section1.ClusterName, "Caves", "server.ini")
	case "master_world":
		FilePath = filepath.Join(GlobalConf.Section1.StorageRoot, "DoNotStarveTogether", GlobalConf.Section1.ClusterName, "Master", "worldgenoverride.lua")
	case "caves_world":
		FilePath = filepath.Join(GlobalConf.Section1.StorageRoot, "DoNotStarveTogether", GlobalConf.Section1.ClusterName, "Caves", "worldgenoverride.lua")
	case "master_mod":
		FilePath = filepath.Join(GlobalConf.Section1.StorageRoot, "DoNotStarveTogether", GlobalConf.Section1.ClusterName, "Master", "modoverrides.lua")
	case "caves_mod":
		FilePath = filepath.Join(GlobalConf.Section1.StorageRoot, "DoNotStarveTogether", GlobalConf.Section1.ClusterName, "Caves", "modoverrides.lua")
	case "setup":
		FilePath = GlobalConf.Section1.ModLuaTarget
	default:
		HttpErr(w, 400, []byte(`{"status":"error", "message":"无效的 target 参数"}`))
		return
	}
	...
	if stat, err := f.Stat(); err == nil {
		if stat.Size() > 1024*1024 {
			HttpErr(w, 413, []byte(`{"status":"error", "message":"file > 1MB, stream rejected"}`))
			return
		}
...
func api_file_write(w http.ResponseWriter, r *http.Request) {
	...
	if r.ContentLength == 0 {
		HttpErr(w, 400, []byte(`{"status":"error", "message":"empty payload rejected"}`))
		return
	}

	Target := strings.ToLower(r.URL.Query().Get("target"))
	var TargetWritePath []string

	switch Target {
	case "cluster":
		TargetWritePath = append(TargetWritePath, filepath.Join(GlobalConf.Section1.StorageRoot, "DoNotStarveTogether", GlobalConf.Section1.ClusterName, "cluster.ini"))
	case "master_server":
		TargetWritePath = append(TargetWritePath, filepath.Join(GlobalConf.Section1.StorageRoot, "DoNotStarveTogether", GlobalConf.Section1.ClusterName, "Master", "server.ini"))
	case "caves_server":
		TargetWritePath = append(TargetWritePath, filepath.Join(GlobalConf.Section1.StorageRoot, "DoNotStarveTogether", GlobalConf.Section1.ClusterName, "Caves", "server.ini"))
	case "master_world":
		TargetWritePath = append(TargetWritePath, filepath.Join(GlobalConf.Section1.StorageRoot, "DoNotStarveTogether", GlobalConf.Section1.ClusterName, "Master", "worldgenoverride.lua"))
	case "caves_world":
		TargetWritePath = append(TargetWritePath, filepath.Join(GlobalConf.Section1.StorageRoot, "DoNotStarveTogether", GlobalConf.Section1.ClusterName, "Caves", "worldgenoverride.lua"))

	case "mod":
		if GlobalConf.Section2.EnableMaster.Load() {
			TargetWritePath = append(TargetWritePath, filepath.Join(GlobalConf.Section1.StorageRoot, "DoNotStarveTogether", GlobalConf.Section1.ClusterName, "Master", "modoverrides.lua"))
		}
		if GlobalConf.Section2.EnableCaves.Load() {
			TargetWritePath = append(TargetWritePath, filepath.Join(GlobalConf.Section1.StorageRoot, "DoNotStarveTogether", GlobalConf.Section1.ClusterName, "Caves", "modoverrides.lua"))
		}
	case "setup":
		TargetWritePath = append(TargetWritePath, GlobalConf.Section1.ModLuaTarget)
	...
	if len(TargetWritePath) == 0 {
		HttpErr(w, 400, []byte(`{"status":"error", "message":"no active shard configured on this node"}`))
		return
	}
	...
	if err := AtomicWriteStream(TargetWritePath[0], r.Body); err != 0 {
		LogOutLn(S2B("致命异常: 流式落盘失败 "), S2B(filepath.Base(TargetWritePath[0])), (S2B(" ")))
		HttpErr(w, 500, []byte(`{"status":"error", "message":"primary write failed"}`))
		return
	}

	if len(TargetWritePath) > 1 {
		for i := 1; i < len(TargetWritePath); i++ {
			if _, err := CloneFile(TargetWritePath[0], TargetWritePath[i]); err != 0 {
				LogOutLn(S2B("[sys] clone failed "), S2B(filepath.Base(TargetWritePath[i])))
				HttpErr(w, 500, []byte(`{"status":"error", "message":"clone to secondary failed"}`))
				return
			}
		}
	}
...
func api_update_state(w http.ResponseWriter, r *http.Request) {
	...
	if TotalRead == 0 {
		HttpErr(w, 400, S2B(`{"status":"error"}`))
		return
	}
...
func ParseApiUint(Payload []byte) uint32 {
	...
	for _, Char := range Payload {
		if Char >= '0' && Char <= '9' {
			Result = Result*10 + uint32(Char-'0')
			ValidNum = true
		} else {
			return 4294967295
		}
...
func ParseApiInt(Payload []byte) int32 {
	...
	var Result int32 = 0
	ValidNum := false
	for i := StartPos; i < len(Payload); i++ {
		Char := Payload[i]
		if Char >= '0' && Char <= '9' {
...
func AtomicWriteStream(TargetPath string, SrcStream io.Reader) uint8 {
	...
	_, err = io.Copy(TempFile, SrcStream)
	if err != nil {
		TempFile.Close()
